...
public class OwnershipHandler extends BaseTypeHandler<Ownership> {
    private Class<Ownership> status;

    private final Ownership[] enums;

    public OwnershipHandler(Class<Ownership> status) {
        if (status == null) {
    ...
    @Override
    public void setNonNullParameter(PreparedStatement preparedStatement, int i, Ownership status, JdbcType jdbcType) throws SQLException {
        preparedStatement.setInt(i, status.getValue());
    }

    @Override
    public Ownership getNullableResult(ResultSet resultSet, String columnName) throws SQLException {
    ...
    @Override
    public Ownership getNullableResult(ResultSet resultSet, int columnIndex) throws SQLException {
        //根据数据库存储类型决定获取类型,Assign数据库中存放int类型
    ...
    @Override
    public Ownership getNullableResult(CallableStatement callableStatement, int columnIndex) throws SQLException {
     ...
     * @return code对应的枚举类
     */
    private Ownership locateEnumStatus(int code) {
        for (Ownership status : enums) {
