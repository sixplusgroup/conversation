...
public class WechatUtil {
    ...
    public static String queryOauthOpenId(String appid, String secret, String code) {
        String result = "";
        String url = new StringBuffer("https://api.weixin.qq.com/sns/oauth2/access_token?appid=").append(appid).append("&secret=").append(secret).append("&code=").append(code).append("&grant_type=authorization_code").toString();
        try {
            URL address = new URL(url);
            HttpURLConnection connection = (HttpURLConnection) address
                    .openConnection();
