...
import com.gmair.shop.api.security.AuthenticationToken;
...
import com.gmair.shop.security.handler.LoginAuthSuccessHandler;
...
public class UserRegisterController {
    ...
    public ResponseEntity<Void> register(HttpServletRequest request, HttpServletResponse response, @Valid @RequestBody UserRegisterParam userRegisterParam) {
        ...
        // 小程序注册/绑定手机号
            ...
            GmairUser gmairUser =  SecurityUtils.getUser();

            appConnect = appConnectService.getByBizUserId(gmairUser.getBizUserId(), App.instance(gmairUser.getAppType()));
            // 通过微信手机号校验
            if (Objects.equals(2, userRegisterParam.getValidateType())) {
                try {
                    WxMaPhoneNumberInfo wxMaPhoneNumberInfo = wxConfig.wxMaService().getUserService().getPhoneNoInfo(gmairUser.getSessionKey(), userRegisterParam.getEncryptedData(), userRegisterParam.getIvStr());
...
//				if (count > 0) {
//					throw new GmairShopBindException("该账号已被绑定，请换个账号试试");
//				}
//			}
        ...
        //进行授权登录
        UserDetails userDetails = gmairUserDetailsService.getGmairUser(userRegisterParam.getAppType(),user, bizUserId);
