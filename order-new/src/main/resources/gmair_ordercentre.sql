...
CREATE TABLE `trade`
(
    `trade_id`          varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL COMMENT '主键id',
    ...
    `status`            varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '枚举类值：交易状态。\r\n可选值: \r\nTRADE_NO_CREATE_PAY(没有创建支付宝交易) \r\n\r\nWAIT_BUYER_PAY(等待买家付款) \r\n\r\nSELLER_CONSIGNED_PART(卖家部分发货) \r\n\r\nWAIT_SELLER_SEND_GOODS(等待卖家发货,即:买家已付款)\r\n\r\nWAIT_BUYER_CONFIRM_GOODS(等待买家确认收货,即:卖家已发货)\r\n\r\nTRADE_BUYER_SIGNED(买家已签收,货到付款专用)\r\n\r\nTRADE_FINISHED(交易成功)\r\n\r\nTRADE_CLOSED(付款以后用户退款成功，交易自动关闭)\r\n\r\nTRADE_CLOSED_BY_TAOBAO(付款以前，卖家或买家主动关闭交易)\r\n\r\nPAY_PENDING(国际信用卡支付付款确认中)\r\n\r\nWAIT_PRE_AUTH_CONFIRM(0元购合约中)\r\n\r\nPAID_FORBID_CONSIGN(拼团中订单或者发货强管控的订单，已付款但禁止发货)',
    `type`              varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '枚举类值：交易类型列表。\r\n\r\n同时查询多种交易类型可用逗号分隔。\r\n默认同时查询guarantee_trade, auto_delivery, ec, cod的4种交易类型的数据\r\n\r\n可选值：\r\nfixed(一口价)\r\nauction(拍卖)\r\nguarantee_trade(一口价、拍卖)\r\nauto_delivery(自动发货)\r\nindependent_simple_trade(旺店入门版交易)\r\nindependent_shop_trade(旺店标准版交易)\r\nec(直冲)\r\ncod(货到付款)\r\nfenxiao(分销)\r\ngame_equipment(游戏装备)\r\nshopex_trade(ShopEX交易)\r\nnetcn_trade(万网交易)\r\nexternal_trade(统一外部交易)\r\no2o_offlinetrade（O2O交易\r\nstep (万人团)\r\nnopaid(无付款订单)\r\npre_auth_type(预授权0元购机交易)',
    `shipping_type`     varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '枚举类型：创建交易时的物流方式（交易完成前，物流方式有可能改变，但系统里的这个字段一直不变）。\r\n\r\n可选值：\r\nfree(卖家包邮)\r\npost(平邮)\r\nexpress(快递)\r\nems(EMS)\r\nvirtual(虚拟发货)\r\n25(次日必达)\r\n26(预约配送)。',
    `trade_from`        varchar(30) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '枚举类值：交易内部来源。\r\n\r\nWAP(手机)\r\nHITAO(嗨淘)\r\nTOP(TOP平台)\r\nTAOBAO(普通淘宝)\r\nJHS(聚划算)\r\n\r\n一笔订单可能同时有以上多个标记，则以逗号分隔',
    `step_trade_status` varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '枚举类值：分阶段付款的订单状态（例如万人团订单等），目前有三返回状态：\r\n\r\nFRONT_NOPAID_FINAL_NOPAID(定金未付尾款未付)\r\nFRONT_PAID_FINAL_NOPAID(定金已付尾款未付)\r\nFRONT_PAID_FINAL_PAID(定金和尾款都付)',
    ...
    `receiver_name`     varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的姓名',
    `receiver_state`    varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的所在省份',
    `receiver_address`  varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的详细地址',
    `receiver_zip`      varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的邮编',
    `receiver_mobile`   varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的手机号码',
    `receiver_phone`    varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的电话号码',
    `receiver_country`  varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人国籍',
    `receiver_city`     varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的所在城市\n注：因为国家对于城市和地区的划分的有：省直辖市和省直辖县级行政区（区级别的）划分的，淘宝这边根据这个差异保存在不同字段里面比如：广东广州：广州属于一个直辖市是放在的receiver_city的字段里面；而河南济源：济源属于省直辖县级行政区划分，是区级别的，放在了receiver_district里面\n建议：程序依赖于城市字段做物流等判断的操作，最好加一个判断逻辑：如果返回值里面只有receiver_district参数，该参数作为城市',
    `receiver_district` varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人的所在地区\n注：因为国家对于城市和地区的划分的有：省直辖市和省直辖县级行政区（区级别的）划分的，淘宝这边根据这个差异保存在不同字段里面比如：广东广州：广州属于一个直辖市是放在的receiver_city的字段里面；而河南济源：济源属于省直辖县级行政区划分，是区级别的，放在了receiver_district里面\n建议：程序依赖于城市字段做物流等判断的操作，最好加一个判断逻辑：如果返回值里面只有receiver_district参数，该参数作为城市',
    `receiver_town`     varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '收货人街道地址',
    `tmall_delivery`    tinyint(1)                                             NULL DEFAULT NULL COMMENT '是否为tmallDelivery',
    `cn_service`        varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '天猫直送服务',
    `delivery_cps`      varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '派送CP（运送公司？）',
    `cutoff_minutes`    varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '截单时间',
  ...
  CHARACTER SET = utf8
  COLLATE = utf8_general_ci COMMENT = '主订单表'
  ROW_FORMAT = Dynamic;
...
CREATE TABLE `order`
(
    `order_id`            varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci  NOT NULL COMMENT '主键id',
    `trade_id`            varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT 'trade_id from trade表',
    ...
    `status`              varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '字典项：订单状态（请关注此状态，如果为TRADE_CLOSED_BY_TAOBAO状态，则不要对此订单进行发货，切记啊！）。可选值:\nTRADE_NO_CREATE_PAY(没有创建支付宝交易)\nWAIT_BUYER_PAY(等待买家付款)\nWAIT_SELLER_SEND_GOODS(等待卖家发货,即:买家已付款)\nWAIT_BUYER_CONFIRM_GOODS(等待买家确认收货,即:卖家已发货)\nTRADE_BUYER_SIGNED(买家已签收,货到付款专用)\nTRADE_FINISHED(交易成功)\nTRADE_CLOSED(付款以后用户退款成功，交易自动关闭)\nTRADE_CLOSED_BY_TAOBAO(付款以前，卖家或买家主动关闭交易)\nPAY_PENDING(国际信用卡支付付款确认中)',
    ...
    `sku_properties_name` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT 'SKU的值。如：机身颜色:黑色;手机套餐:官方标配',
    `seller_type`         varchar(2) CHARACTER SET utf8 COLLATE utf8_general_ci   NULL DEFAULT NULL COMMENT '卖家类型，可选值为：B（商城商家），C（普通卖家）',
    `refund_status`       varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '枚举类值：退款状态。\r\n\r\n可选值：\r\nWAIT_SELLER_AGREE(买家已经申请退款，等待卖家同意)\r\nWAIT_BUYER_RETURN_GOODS(卖家已经同意退款，等待买家退货)\r\nWAIT_SELLER_CONFIRM_GOODS(买家已经退货，等待卖家确认收货)\r\nSELLER_REFUSE_BUYER(卖家拒绝退款)\r\nCLOSED(退款关闭)\r\nSUCCESS(退款成功)',
    ...
    `pic_path`            varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '商品图片的绝对路径',
    `store_code`          varchar(20) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '发货的仓库编码',
    `shipping_type`       varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '子订单的运送方式（卖家对订单进行多次发货之后，一个主订单下的子订单的运送方式可能不同，用order.shipping_type来区分子订单的运送方式）',
    `logistics_company`   varchar(10) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '子订单发货的快递公司名称',
    `invoice_no`          varchar(15) CHARACTER SET utf8 COLLATE utf8_general_ci  NULL DEFAULT NULL COMMENT '子订单所在包裹的运单号',
...
(
    item_id         varchar(45)          not null comment '主键'
        primary key,
    num_iid         varchar(45)          not null comment '商品id',
    sku_id          varchar(45)          null comment '商品的最小库存单位Sku的id',
    title           varchar(45)          not null comment '商品标题,不能超过60字节',
    ...
    constraint sku_item_sku_id_uindex unique (sku_id)
) ENGINE = INNODB
  DEFAULT CHARSET = UTF8 comment '商品项表';
...
(
    user_id          varchar(45)  not null comment '主键'
        primary key,
    start_sync_time  datetime     not null comment '同步开始时间',
    last_update_time datetime     null comment '上次同步时间',
    session_key      varchar(100) not null comment '用户授权token',
    authorize_time   datetime     not null comment '用户授权时间',
    block_flag       tinyint(1)   not null,
    create_at        datetime     not null
) ENGINE = INNODB
  DEFAULT CHARSET = UTF8 comment '淘宝卖家用户表';

insert into tb_user(user_id, start_sync_time, last_update_time, session_key, authorize_time, block_flag, create_at)
    value ("SEL2020102088rn7213", "2020-10-01 00:00:00", null,
           "6100e02ceb111ceb4e6ff02506458185b2f7afa5fce9f232200642250842", "2020-10-01 00:00:00", false, now())
