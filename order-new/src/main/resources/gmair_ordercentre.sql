...
create table trade
(
    trade_id          varchar(20) not null comment '主键id',
    ...
    status            varchar(20) null comment '枚举类值：交易状态。
可选值:
...
PAID_FORBID_CONSIGN(拼团中订单或者发货强管控的订单，已付款但禁止发货)',
    type              varchar(10) null comment '枚举类值：交易类型列表。
...
pre_auth_type(预授权0元购机交易)',
    shipping_type     varchar(10) null comment '枚举类型：创建交易时的物流方式（交易完成前，物流方式有可能改变，但系统里的这个字段一直不变）。
...
25(次日必达)
26(预约配送)。',
    trade_from        varchar(30) null comment '枚举类值：交易内部来源。
...
FRONT_PAID_FINAL_PAID(定金和尾款都付)',
    ...
    receiver_name     varchar(10) null comment '收货人的姓名',
    receiver_state    varchar(10) null comment '收货人的所在省份',
    receiver_address  varchar(20) null comment '收货人的详细地址',
    receiver_zip      varchar(10) null comment '收货人的邮编',
    receiver_mobile   varchar(15) null comment '收货人的手机号码',
    receiver_phone    varchar(15) null comment '收货人的电话号码',
    receiver_country  varchar(15) null comment '收货人国籍',
    receiver_city     varchar(15) null comment '收货人的所在城市
...
建议：程序依赖于城市字段做物流等判断的操作，最好加一个判断逻辑：如果返回值里面只有receiver_district参数，该参数作为城市',
    receiver_district varchar(15) null comment '收货人的所在地区
注：因为国家对于城市和地区的划分的有：省直辖市和省直辖县级行政区（区级别的）划分的，淘宝这边根据这个差异保存在不同字段里面比如：广东广州：广州属于一个直辖市是放在的receiver_city的字段里面；而河南济源：济源属于省直辖县级行政区划分，是区级别的，放在了receiver_district里面
建议：程序依赖于城市字段做物流等判断的操作，最好加一个判断逻辑：如果返回值里面只有receiver_district参数，该参数作为城市',
    receiver_town     varchar(20) null comment '收货人街道地址',
...
(
    ...
    trade_id            varchar(20)  null comment 'trade_id from trade表',
    num_iid             bigint       null comment '商品数字ID',
    cid                 bigint       null comment '交易商品对应的类目ID',
    oid                 bigint       null comment '子订单编号',
    status              varchar(20)  null comment '字典项：订单状态（请关注此状态，如果为TRADE_CLOSED_BY_TAOBAO状态，则不要对此订单进行发货，切记啊！）。可选值:
TRADE_NO_CREATE_PAY(没有创建支付宝交易)
WAIT_BUYER_PAY(等待买家付款)
WAIT_SELLER_SEND_GOODS(等待卖家发货,即:买家已付款)
WAIT_BUYER_CONFIRM_GOODS(等待买家确认收货,即:卖家已发货)
...
PAY_PENDING(国际信用卡支付付款确认中)',
    ...
    refund_status       varchar(10)  null comment '枚举类值：退款状态。
...
SUCCESS(退款成功)',
    ...
    store_code          varchar(20)  null comment '发货的仓库编码',
    shipping_type       varchar(10)  null comment '子订单的运送方式（卖家对订单进行多次发货之后，一个主订单下的子订单的运送方式可能不同，用order.shipping_type来区分子订单的运送方式）',
    logistics_company   varchar(10)  null comment '子订单发货的快递公司名称',
    invoice_no          varchar(15)  null comment '子订单所在包裹的运单号',
